...
const inputClass = "h-12 w-full rounded-xl border border-[#7b625d]/20 bg-white/80 px-4 text-[#403735] outline-none transition focus:border-[#b9726f] focus:ring-2 focus:ring-[#efbfbb]/35";
...
export function RegistroForm({ next }: { next?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="rounded-[28px] border border-white/70 bg-[#fffdf9]/95 p-5 shadow-[0_28px_70px_rgba(80,59,54,.14)] backdrop-blur md:p-8">
      ...
      <p className="mt-5 flex items-start justify-center gap-2 text-center text-xs leading-5 text-[#7e716d]"><LockKeyhole className="mt-0.5 h-3.5 w-3.5 shrink-0" />Tus datos se usan solo para la organización de la boda.</p>
    </form>
  );
}

function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return <label className="block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-[.18em] text-[#7b625d]">{label}</span>{children}{error && <span className="mt-1 block text-xs text-[#a44949]">{error}</span>}</label>;
}
